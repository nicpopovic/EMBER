...
from src.data.ner import get_IOB_per_token, CollateManagerExp1, get_features_and_labels_exp5
...
parser.add_argument("--data_dir", type=str, default="featurecache/exp_05", help="dataset")
...
parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/exp_05", help="checkpoint_dir")
parser.add_argument("--project_name", type=str, default="2311_embedIE_05_NER_span_classification", help="project_name")
...
for split in ["train", "validation", "test"]:
    ...
    try:
        ...
        dataloader_gen = DataLoader(dataset_gen, batch_size=batch_size, shuffle=False, collate_fn=CollateManagerExp1(dataset_id).collateExp5)


        # ----- create features and labels

        features_gen, labels_gen = get_features_and_labels_exp5(model, dataloader_gen)
            ...
            if loadername == "test":
                ...
                for batch in tqdm(dataloader_test_seqeval):
                    ...
                    for ind in indices:
                        ...
                        check = assigned[index[0], index[2]:index[1]+1].sum()
                        if check > 0 or index[1] >= batch['lens'][index[0]]:
                            continue
                        assigned[index[0], index[2]:index[1]+1] = 1
                        iob_labels[index[0]][index[2]:index[1]+1] = ["I-SPAN"] * (index[1]+1-index[2])
